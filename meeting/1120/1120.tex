...
%--------------------------------------------------
\begin{frame}[fragile,allowframebreaks]{Handle の役割}
\begin{columns}
  \begin{column}{0.48\textwidth}
\begin{lstlisting}[language=Java,basicstyle=\ttfamily\scriptsize]
<Handle
...
%--------------------------------------------------
\begin{frame}{まとめ：React Flow と CustomNode の関係}
\begin{block}{まとめ}
\begin{itemize}
  \item React Flow は「ノードとエッジ」で構成されるグラフ描画ライブラリ。
  \item ノードの見た目を自由に作りたいとき、\texttt{CustomNode} を定義する。
  \item \texttt{Handle} によりノード同士を接続可能。
  \item 状態管理（選択・移動・接続）は React Flow が自動で行う。
\end{itemize}
\end{block}
\end{frame}
