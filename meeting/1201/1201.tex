...
\section{概要}
\begin{frame}{前回までの進捗状況}
	\begin{enumerate}
		\setlength{\parskip}{1.5em}
		\item ○×問題だけでなく,数値の入力問題でも正答判定を可能化.
		\item それぞれの問題の正解・不正解を保存.
		\item React Flowによるグラフ描画の調整.
	\end{enumerate}
\end{frame}
...
\section{カテゴリ別の解答欄の作成}
\begin{frame}[fragile]{データベースにカテゴリ属性追加}
	\begin{columns}
...
\end{lstlisting}
	...
	\end{columns}
	\vspace{1em}
	\centering \Rightarrow \ \ カテゴリ別に解答欄を作成へ\\
\end{frame}

\begin{frame}[fragile,allowframebreaks]{case文による解答欄の判別}
	\begin{lstlisting}[language=Java,basicstyle=\ttfamily\scriptsize]
function renderAnswerUI(p: Problem) {
  switch (p.category) {
    case ProblemCategory.TrueOrFalse: // ○×問題の場合
      return (
        <div>
          <button
...
// 入力問題の場合は入力された値を保存・送信する関数 handleAnswer を作成.
    case ProblemCategory.Input: // 入力問題の場合
      return (
        <InputProblem
          p={p}
    ...
    case ProblemCategory.Choice: // 選択問題の場合
        //まだ選択肢を DB にどう入れるか未検討につき未完成 
      return null;
...
\begin{frame}[fragile]{1問ずつ正答判定欄を作成（problems/page.tsx）}
  ...
  \end{lstlisting}
	...
	\centering $\implies$しかし,正答判定結果が問題ごとに保存されていない.\\
\end{frame}

\begin{frame}[fragile,allowframebreaks]{問題別の正答結果を維持させる（problems/page.tsx）}
	\begin{enumerate}
		\setlength{\parskip}{1.5em}
		\small{
		\item React Hookと呼ばれる状態管理機能（useState）を用いる\cite{typescript}.
		\item それぞれの問題の正解を保持できるmap（messages）と更新関数（setMessages）を作成.
		\item サーバーから用いた正答判定をmessagesに保存させる.
		      }
	\end{enumerate}
...
\begin{frame}{実行結果}
	\begin{columns}
		\begin{column}{0.48\textwidth}
			\begin{figure}
				\fbox{\includegraphics[scale=0.25]{judge1.png}}
				\caption{修正前}
			\end{figure}
		\end{column}
		...
		\begin{column}{0.48\textwidth}
			\begin{figure}
				\fbox{\includegraphics[scale=0.25]{judge2.png}}
				\caption{修正後}
			\end{figure}
		\end{column}
	\end{columns}
	\centering \textbf{\rightarrow \ 個別に正答判定を出力・保持できるようになった.}
\end{frame}
...
\begin{frame}{Prisma CLI（Prisma command line interface）とは}
	\begin{itemize}
		\setlength{\parskip}{1em}
		\item Prismaの各種操作をターミナルから実行するためのコマンド群\cite{prisma}．
...
\begin{frame}{今回の進捗状況}
	...
	\vspace{1.5em}
	\textbf{\rightarrow 残りの3つと◎×問題以外にも対応可にさせることに取り組む.}
\end{frame}
%--------------------------------------------------
